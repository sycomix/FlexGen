...
class SlurmRunner(MultiNodeRunner):
    ...
    def get_cmd(self, environment, active_resources):
        ...
        if self.args.include != "":
            srun_cmd.extend(('--include', f'{self.args.include}'))
        if self.args.exclude != "":
            srun_cmd.extend(('--exclude', f'{self.args.exclude}'))
        if self.args.num_nodes > 0:
            srun_cmd.extend(('--nodes', f'{self.args.num_nodes}'))
        if self.args.num_gpus > 0:
            srun_cmd.extend(('--gpus', f'{self.args.num_gpus}'))
        exports = '--export=ALL'
